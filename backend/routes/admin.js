...
router.get('/stats', isAdmin, async (req, res) => {
  try {
    const [users, residences, incidents, charges] = await Promise.all([
    ...
    const userStats = {};
    users.rows.forEach(r => { userStats[r.role] = +r.cnt; });
    const incStats = {};
    incidents.rows.forEach(r => { incStats[r.statut] = +r.cnt; });
    res.json({
      totalUsers: users.rows.reduce((s, r) => s + +r.cnt, 0),
      userStats,
      totalResidences: +residences.rows[0]?.cnt || 0,
      incStats,
...
router.get('/users', isAdmin, async (req, res) => {
  try {
    const { rows } = await query(
      `SELECT u.id, u.email, u.prenom, u.nom, u.role, u.lot, u.tantiemes,
              u.telephone, u.created_at, r.nom AS residence_nom
       FROM utilisateurs u
  ...
  try {
    ...
    const { rows } = await query(
      `INSERT INTO utilisateurs (residence_id,email,password_hash,prenom,nom,telephone,lot,tantiemes,role)
       ...
       RETURNING id,email,prenom,nom,role,lot,tantiemes,telephone`,
...
router.put('/users/:id', isAdmin, async (req, res) => {
  ...
  try {
    let passwordClause = '';
    const params = [prenom, nom, email, telephone, lot, tantiemes || 0, role, residence_id || 1, req.params.id];
    if (password) {
      const hash = await bcrypt.hash(password, 10);
      passwordClause = ', password_hash=$10';
      params.push(hash);
    }
    const { rows } = await query(
      `UPDATE utilisateurs SET prenom=$1,nom=$2,email=$3,telephone=$4,
       lot=$5,tantiemes=$6,role=$7,residence_id=$8${passwordClause}
       WHERE id=$9 RETURNING id,email,prenom,nom,role,lot,tantiemes,telephone`,
      params
    );
...
router.put('/users/:id/role', isAdmin, async (req, res) => {
  const { role } = req.body;
  const validRoles = ['resident', 'gestionnaire', 'admin'];
  if (!validRoles.includes(role)) return res.status(400).json({ error: 'Rôle invalide' });
  try {
    const { rows } = await query(
      `UPDATE utilisateurs SET role=$1 WHERE id=$2 RETURNING id,email,prenom,nom,role`,
      [role, req.params.id]
    );
    res.json(rows[0]);
  } catch (e) { res.status(500).json({ error: 'Erreur serveur' }); }
...
router.get('/types-charges', isAdmin, async (req, res) => {
  try {
    const { rows } = await query(
      `SELECT t.*, r.nom AS residence_nom FROM types_charges t
       LEFT JOIN residences r ON r.id = t.residence_id
...
router.post('/types-charges', isAdmin, async (req, res) => {
  ...
  try {
    const { rows } = await query(
      `INSERT INTO types_charges (residence_id, nom, description) VALUES ($1,$2,$3) RETURNING *`,
      [residence_id || 1, nom, description]
    );
    res.status(201).json(rows[0]);
  } catch (e) { res.status(500).json({ error: 'Erreur serveur' }); }
});
...
router.put('/types-charges/:id', isAdmin, async (req, res) => {
  ...
  try {
    const { rows } = await query(
      `UPDATE types_charges SET nom=$1, description=$2, actif=$3 WHERE id=$4 RETURNING *`,
      [nom, description, actif ?? true, req.params.id]
    );
    res.json(rows[0]);
  } catch (e) { res.status(500).json({ error: 'Erreur serveur' }); }
});
...
router.delete('/types-charges/:id', isAdmin, async (req, res) => {
  try {
    await query(`DELETE FROM types_charges WHERE id=$1`, [req.params.id]);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: 'Erreur serveur' }); }
});
...
router.get('/types-depenses', isAdmin, async (req, res) => {
  try {
    const { rows } = await query(
      `SELECT t.*, r.nom AS residence_nom FROM types_depenses t
       LEFT JOIN residences r ON r.id = t.residence_id
...
router.post('/types-depenses', isAdmin, async (req, res) => {
  ...
  try {
    const { rows } = await query(
      `INSERT INTO types_depenses (residence_id, nom, description, categorie) VALUES ($1,$2,$3,$4) RETURNING *`,
      [residence_id || 1, nom, description, categorie || 'general']
    );
    res.status(201).json(rows[0]);
  } catch (e) { res.status(500).json({ error: 'Erreur serveur' }); }
});
...
router.put('/types-depenses/:id', isAdmin, async (req, res) => {
  ...
  try {
    const { rows } = await query(
      `UPDATE types_depenses SET nom=$1, description=$2, categorie=$3, actif=$4 WHERE id=$5 RETURNING *`,
      [nom, description, categorie || 'general', actif ?? true, req.params.id]
    );
    res.json(rows[0]);
  } catch (e) { res.status(500).json({ error: 'Erreur serveur' }); }
});
...
router.delete('/types-depenses/:id', isAdmin, async (req, res) => {
  try {
    await query(`DELETE FROM types_depenses WHERE id=$1`, [req.params.id]);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: 'Erreur serveur' }); }
});
...
router.get('/residences', isAdmin, async (req, res) => {
  try {
    const { rows } = await query(
      `SELECT r.*, COUNT(u.id) AS nb_utilisateurs
       FROM residences r LEFT JOIN utilisateurs u ON u.residence_id = r.id
