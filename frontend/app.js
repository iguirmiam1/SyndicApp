...
async function generateQR(containerId, data) {
  ...
  try {
    ...
    let realToken = text;
    try { const p=JSON.parse(text); if(p.token) realToken=p.token; } catch(e2){}
    const tokenBlocks = realToken.slice(0,24).toUpperCase().match(/.{1,4}/g)?.join('-') || realToken.slice(0,16);
    el.innerHTML = `
      <div style="background:#0a3d2e;color:#fff;padding:1.5rem 1rem;border-radius:12px;
                  text-align:center;max-width:240px;margin:0 auto">
        <div style="font-size:2.5rem;margin-bottom:.375rem">🎫</div>
        <div style="font-size:.6rem;font-weight:700;letter-spacing:.1em;opacity:.7;margin-bottom:.5rem">CODE D'ACCÈS JASMINE PARK</div>
        <div style="font-size:1rem;font-weight:800;letter-spacing:.06em;font-family:monospace">${tokenBlocks}</div>
        <div style="font-size:.65rem;opacity:.6;margin-top:.625rem">Montrez ce code à la sécurité</div>
      </div>`;
  }
}
...
async function showQRCode(token, resa) {
  ...
  const _prenom = r.prenom || _su.prenom || '';
  const _lot = r.lot || _su.lot || '';
  document.getElementById('qr-info').innerHTML = `
    <div style="text-align:center;margin-bottom:1rem">
      ...
      ${_prenom?`<div style="font-size:.8rem;color:var(--text-3);margin-top:.25rem">${_prenom} · Lot ${_lot}</div>`:''}
    </div>`;
